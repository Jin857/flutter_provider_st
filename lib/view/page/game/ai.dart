...
import 'common.dart' as game;
import 'player.dart';
import 'buffer_container.dart';
...
class AdvancedAI extends AI {
  ...
  num alphaBetaSearch(ChessNode current) {
    ...
    if (current.parentNode != null &&
        !current.parentNode!.childrenNode.contains(current)) {
    ...
    if (current.type == ChildType.MIN) {
      // 当前节点为MIN节点
      num parentMin = current.parentNode?.minValue ??
          double.negativeInfinity; // 获取父节点的最小值，若不存在父节点则设置为负无穷大
      int index = 0; // 索引计数器
      ...
      for (ChessNode node in children) {
        ...
        num newCurrentMax =
            min(current.maxValue, alphaBetaSearch(node)); // 计算当前子节点的最大值

        if (newCurrentMax <= parentMin) {
          // 如果当前子节点的最大值小于等于父节点的最小值，则说明该枝可以被完全剪掉
          current.childrenNode =
              current.childrenNode.sublist(0, index); // 将当前节点的子节点列表截断至当前索引位置
      ...
      // 当前节点为MAX节点
      num parentMax = current.parentNode?.maxValue ??
          double.infinity; // 获取父节点的最大值，若不存在父节点则设置为正无穷大
      int index = 0; // 索引计数器
      ...
      for (ChessNode node in children) {
        ...
        num newCurrentMin =
            max(current.minValue, alphaBetaSearch(node)); // 计算当前子节点的最小值
